...
{
    public partial class frmStockMovements : Form
    {
        ...
        private void frmStockMovements_Load(object sender, EventArgs e)
        {
        ...
        private void LoadStockList()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("[SP_GetStockList]", connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@StockName", txtStock.Text);
        ...
        private void adDgrvDocumentList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                int doc_no = Convert.ToInt32(adDgrvDocumentList.Rows[e.RowIndex].Cells["ID"].Value);

                // Yükləmək üçün LoadDocumentDetails metodunu çağırırıq
                LoadStockDetails(doc_no);
                UpdateTotals();
